...
<body>
    <form class="container" enctype="multipart/form-data" action="academic_year_setup.php" method="post">
        ...
        <header>Approve/Reject Academic Year Setup</header>
        <?php
        include 'connect.php';

        if ($conn) {
            $sql = "select * from academic_calendar where  s_id = $sid and status = 'ACCEPTED' or status = 'PENDING' and status != 'EXPIRED' order by academic_year ";
  ...
  text-decoration: none;" name="generate" type="submit">
        ...
        <?php
             if(isset($_POST['approve'])){
                $sql = oci_parse( $conn,"update academic_calendar set status = 'ACCEPTED' where s_id = $sid and status = 'PENDING' ");
                if(oci_execute($sql)){
                    ?><div style="font-size:15px;
                    color: green;
                    position: relative;
                     display:flex;
                    animation:button .3s linear;text-align: center;">
                        <?php echo "ACADEMIC YEAR APPROVED SUCCESSFULLY";
                            header("refresh:2;"); ?>
                    </div> <?php
                }else {
                    ?><div style="font-size:15px;
                    ...
                    position: relative;
                     display:flex;
                    animation:button .3s linear;text-align: center;">
                        <?php echo "ERROR APPROVING ACADEMIC YEAR";
                            header("refresh:2;"); ?>
                    </div> <?php
                }
             ...
             if(isset($_POST['reject'])){
                $sql = oci_parse( $conn,"DELETE FROM ACADEMIC_CALENDAR WHERE STATUS = 'PENDING' AND S_ID = $sid  ");
                if(oci_execute($sql)){
                    ?><div style="font-size:15px;
                    color: red;
                    position: relative;
                     display:flex;
                    animation:button .3s linear;text-align: center;">
                        <?php echo "ACADEMIC YEAR REJECTED SUCCESSFULLY";
                            header("refresh:2;"); ?>
                    </div> <?php
                }else {
                    ?><div style="font-size:15px;
