                            ...
                            animation:button .3s linear;text-align: center;">
                                ...
                                animation:button .3s linear;text-align: center;">
                                            <?php echo "CLASS LIST FOR $class_name GENERATED";
                                                ...
                                                $query = "SELECT DISTINCT(A.STUD_ID),A.NAME,D.CLASS_NAME FROM STUDENT A  JOIN CLASS_STUDENT C ON (A.STUD_ID=C.STUD_ID) JOIN  SUB_CLASS D ON (C.SUB_CODE=D.SUB_CODE) WHERE C.SUB_CODE = $sub_code and c.s_id = $sid ORDER  BY  A.STUD_ID";
                                            //   echo $query;
                                                // Prepare and execute the query
                                                $statement = oci_parse($conn, $query);
                                                oci_execute($statement);
                                                ...
                                                $sheet->setCellValue('E1', 'CLASS NAME');
                                                $directoryPath = 'C:\ACADEMIX\\' . $school . '\generated_reports\class_list\\';
                                                if (!is_dir($directoryPath)) {
                                                    if (!mkdir($directoryPath, 0777, true)) {
                                                        die('Failed to create directories.');
