...
?>
  ...
  <div class="wrapper">
    ...
    <h2>Grade</h2>
    <form action="select_subject_grade.php" method="post" style="min-height: 250px;">
      <div class="input-box">
        <select required name="subject">
          <option disabled selected>Select Subject</option>
          <?php
          $get_hos = "select a.subject,a.sub_code,c.CLASS_NAME from waec_subject a join teacher_subject b on (a.sub_code=b.sub_code) join sub_class c on (b.s_code=c.sub_code) where b.emp_id= $emp_id";
...
//echo  "select a.SUB_CODE,c.S_CODE from waec_subject a join teacher_subject c on (a.sub_code=c.sub_code) join sub_class d on (c.s_code=d.sub_code) where c.emp_id= $emp_id and a.subject = '$subject' and d.class_name = '$className' ";
        ...
        ?><div style="font-size:15px;
            ...
            <?php echo "STUDENT EVALUATION FOR $class_name TAKING $subject ";
            header("refresh:2;url=grade_settings.php"); 
          } else {
            ?><div style="font-size:15px;
                    color: red;
                    position: relative;
