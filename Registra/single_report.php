            ...
            if (isset($_POST['generate_classlist'])) {
                ...
                if (isset($_POST['report_type'])) {
                    ...
                    if ($rpt_type == 'PDF') {
                        if (isset($_POST['class_name'])) {
                            ...
                            foreach ($stud_ids as $st) {
                                ...
                                $pdf->SetFont('times', '', 6); // Adjust the font size as needed

                                //  $pdf->Cell(0, 5, $t, 0, 1, 'R'); // Replace $academicYear with the actual variable holding the value
  
                                  $pdf->SetX(150);
                                  $sql = oci_parse($conn, "select * from academic_calendar a join term_calendar b on (a.academic_year=b.academic_year) where b.status='ACTIVE' is not null order by b.term desc");
                                  oci_execute($sql);
                                  if ($row = oci_fetch_array($sql)) {
                                      $a_y = $row['ACADEMIC_YEAR'];
                                    //  $t = $row['TERM'];
                                   $pdf->Cell(0, 5, $row['TERM'], 0, 1, 'R');
                                  }
                    ...
                    WHERE TRIM(C.STUD_ID) = TRIM('$st')
                      ...
                      AND C.CLASS_CODE = '$sub_code'
                      AND D.SUB_CODE = '$sub_code'
                    ORDER BY A.SUBJECT ";
