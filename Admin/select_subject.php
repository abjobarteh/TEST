...
<body>
  <div class="wrapper">
    ...
    <h2>Mark Entry</h2>
    <form action="select_subject.php" method="post" style="min-height: 250px;">
      <div class="input-box">
        <select required name="subject">
          <option disabled selected>Select Subject</option>
          <?php
  $get_hos = "select DISTINCT(A.SUB_CODE),a.subject,c.CLASS_NAME from waec_subject a join teacher_subject b on (a.sub_code=b.sub_code) join sub_class c on (b.s_code=c.sub_code) where b.emp_id= $emp_id order by a.subject,c.class_name";
        ...
        ?><div style="font-size:15px;
            ...
            <?php echo '<script>
                                  Swal.fire({
                                      position: "center",
                                      icon: "success",
                                      title: "MARKS TO BE ENTERED FOR ' . $class_name . ' TAKING '. $subject.'",
                                      showConfirmButton: false,
                                      timer: 1500
                                  });
                              </script>';
            header("refresh:2;url=student_evaluation.php"); 
          } else {
            ?><div style="font-size:15px;
                    color: red;
                    position: relative;
