...
<body>
    <form class="container" enctype="multipart/form-data" action="unprocess.php" method="post">
        ...
        <div class="input-field">
            ...
            if ($conn) {
                $status = 'PENDING';
                $sql = "SELECT E.NAME, C.SUBJECT, B.CLASS_NAME, A.CONST_ASS, A.EXAM, (A.CONST_ASS * 100 / 30) AS CASS, (A.EXAM * 100 / 70) AS EXAM, A.ENTRY_DT, D.NAME AS MARKENTEREDBY, A.MARK_STATUS AS STATUS FROM STUDENT_EVALUATION A JOIN SUB_CLASS B ON (A.CLASS_CODE = B.SUB_CODE) JOIN WAEC_SUBJECT C ON (A.SUB_CODE = C.SUB_CODE) JOIN EMPLOYEE D ON (A.EMP_ID = D.EMP_ID) JOIN STUDENT E ON (A.STUD_ID = E.STUD_ID) WHERE B.SUB_CODE = '$s_code' AND A.MARK_STATUS = '$status' ORDER BY B.CLASS_NAME, C.SUBJECT, E.NAME";
                //echo $sql;
  ...
  text-decoration: none;" name="generate" type="submit">
            ...
            if (isset($_POST['generate'])) {
                $status = 'PENDING';
                $query = "SELECT E.NAME, C.SUBJECT, B.CLASS_NAME, A.CONST_ASS, A.EXAM, (A.CONST_ASS * 100 / 30) AS CASS, (A.EXAM * 100 / 70) AS EXAM, A.ENTRY_DT, D.NAME AS MARKENTEREDBY, A.MARK_STATUS AS STATUS FROM STUDENT_EVALUATION A JOIN SUB_CLASS B ON (A.CLASS_CODE = B.SUB_CODE) JOIN WAEC_SUBJECT C ON (A.SUB_CODE = C.SUB_CODE) JOIN EMPLOYEE D ON (A.EMP_ID = D.EMP_ID) JOIN STUDENT E ON (A.STUD_ID = E.STUD_ID) WHERE B.SUB_CODE = '$s_code' AND A.MARK_STATUS = '$status' ORDER BY B.CLASS_NAME, C.SUBJECT, E.NAME";
