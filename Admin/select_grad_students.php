...
?>
  ...
  <div class="wrapper">
    ...
    <h2>Class</h2>
    <form action="select_grad_students.php" method="post" style="min-height: 250px;">
      <div class="input-box">
      <select required name="teacher">
                <option disabled selected>Select Class</option>
                <?php
                $get_hos = "SELECT DISTINCT(C.CLASS_NAME),C.SUB_CODE FROM STUDENT A JOIN STUDENT_ACADEMIC B ON (A.STUD_ID=B.STUD_ID) JOIN SUB_CLASS C ON (B.SUB_CODE=C.SUB_CODE) WHERE A.STATUS != 'GRADUATED' AND C.SUB_CODE != 260  AND C.SUB_CODE != 266 AND C.SUB_CODE != 321 AND A.S_ID =$sid   ORDER BY C.CLASS_NAME";
                    ...
                    while($r= oci_fetch_array($sql)) {
                     ...
                     animation:button .3s linear;text-align: center;">
                                     <?php echo "SELECTED CLASS: $class_name ";
                                         header("refresh:2;url=show_graduated_students.php"); ?>
                                 </div> <?php
                    }
