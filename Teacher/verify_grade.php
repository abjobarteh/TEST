...
//echo "select * from academic_calendar a join term_calendar b on (a.academic_year=b.academic_year) where b.start_dt is not null and b.term = '$term' ";
        ...
        $sql = "SELECT * FROM STUDENT A JOIN STUDENT_EVALUATION B ON (A.STUD_ID=B.STUD_ID) WHERE B.S_ID = $sid AND MARK_STATUS = 'PENDING' and B.CLASS_CODE = $s_code AND B.SUB_CODE = $sub_code AND B.EMP_ID = '$emp_id' AND B.TERM ='$t' ORDER BY A.NAME ";
       //  echo $sql ;
  ...
  text-decoration: none;" name="reject" type="submit">
            ...
            if (isset($_POST['enroll']) && !empty($_POST['enroll'])) {
                ...
                foreach ($selectedStudentIds as $studentId) {
                    ...
                    if (oci_execute($sql)) {
                        ...
                        $gettotal = "SELECT * FROM STUDENT A JOIN STUDENT_EVALUATION B ON (A.STUD_ID=B.STUD_ID) WHERE B.S_ID = $sid AND MARK_STATUS = 'ACCEPTED' and B.CLASS_CODE = $s_code AND B.SUB_CODE = $sub_code AND B.EMP_ID = '$emp_id' AND B.STUD_ID = '$stud_id' AND B.TERM='$t'  ";
        ...
        <?php
        require 'C:\wamp64\www\Academix\KOTU SENIOR SECONDARY SCHOOL\Registra\PHPMailer.php';
        require 'C:\wamp64\www\Academix\KOTU SENIOR SECONDARY SCHOOL\Registra\Exception.php';
        require 'C:\wamp64\www\Academix\KOTU SENIOR SECONDARY SCHOOL\Registra\SMTP.php';
