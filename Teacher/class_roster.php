...
<body>
    <form class="container" enctype="multipart/form-data" action="class_roster.php" method="post">
        ...
        $sql = oci_parse($conn, "SELECT * FROM CLASS_TEACHER WHERE EMP_ID = '$emp_id' and s_id = $sid ");
        oci_execute($sql);
        if ($rowS = oci_fetch_array($sql)) {
            $s_code = $rowS["SUB_CODE"];
        }
        $sql = "SELECT * FROM STUDENT A JOIN CLASS_STUDENT B ON (A.STUD_ID=B.STUD_ID) JOIN STUDENT_PERSONAL C ON (A.STUD_ID=C.STUD_ID) WHERE B.SUB_CODE = $s_code and A.STATUS !='GRADUATED' and A.S_ID = $sid ORDER BY A.NAME";
        //  echo $sql;
